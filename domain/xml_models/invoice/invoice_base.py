...
from domain.dtos.invoice_dto import TaxSubTotalDto
from shared import templates_loader

class InvoiceBase:
    def __init__(self):
        self.root = etree.fromstring(templates_loader.template.xml_invoice.encode('utf-8'))
    ...
    def add_invoice_line(self, line: dict):
        parent = self.root.xpath('//cac:Invoice', namespaces=self.names)
        cac = '{' + self.names["cac"] + '}'
        cbc = '{' + self.names["cbc"] + '}'

        parent = self.root.xpath('//ns:Invoice', namespaces={'ns': 'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2'})
        line_xml = etree.Element(f'{cac}InvoiceLine')
